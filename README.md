...
### v1.3.1
**버그 수정**
- 🐛 GIF 스프라이트 비교 시 `.src` → `getAttribute("src")` (GIF 재시작 방지)
- 🐛 XSS 방지: `innerHTML` → `createElement` + `textContent`
- 🐛 투명도 설정이 2번째 펫에 공유되지 않던 문제
- 🐛 걷기 중 위치가 설정에 저장되어 새로고침 시 이상한 위치에 표시
- 🐛 충돌 보정 후 위치가 저장되지 않던 문제
- 🐛 iPadOS (Macintosh UA) 태블릿 감지 실패
- 🐛 자동 일기 시 2번째 펫 `isPetGenerating` 체크 누락

**개선**
- 🍖 배고픔 게이지 감소 속도 완화 (5분당 -3 → -1, 약 8.3시간에 0)
