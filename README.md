...
- **말투 언어 선택**: 한국어 / 영어 / 일본어 / 중국어 — AI 프롬프트 언어 설정

### 🐱🐱 멀티펫 시스템
- **2마리 동시 표시**: 저장된 프리셋 중 하나를 2번째 펫으로 불러오기
- **채팅 반응 분배**: 1번째만 / 2번째만 / 번갈아가며
- **듀얼 직접대화**: 말걸기 시 양쪽 펫이 1회 API 호출로 동시 반응
- **펫끼리 자동 대화**: 설정 간격마다 두 펫이 자동으로 대화 (로그 기록 + 기억)
- **펫 충돌 감지**: 겹치면 자동으로 밀어내기 + 충돌 대사
- **2번째 펫 위치 프리셋**: 별도 위치 설정 가능
- **독립적 컨디션**: 2번째 펫도 배고픔 게이지, 밥주기 독립 관리
...
- 멀티펫: 2번째 펫도 독립적인 말풍선 (프리셋별 디자인 유지)

### 📝 커스텀 대사 (13가지 상황)
- idle, sleeping, dragging, click, clickSpam, greeting, latenight, morning, longAbsence, feeding, hungry, petting, **collision** (멀티펫 충돌)
- 각 상황별 여러 대사 등록 가능
...
- **2번째 펫 독립 관리**: 멀티펫 사용 시 각각 별도 배고픔 게이지
- **3단계 배고픔 패널티**:
...
- 1시간+ 오래 비움 시 특별 인사
- 드래그 중 전용 반응
- **API 충돌 처리**: 채팅 반응은 최대 15초 대기, 펫간 대화는 30초 후 1회 재시도

### 💾 프리셋 시스템
- 저장 / 불러오기 / 덮어쓰기 / 삭제
- JSON 내보내기/가져오기 (프리셋 공유)
- 개인정보 자동 제거 (ownerName, ownerPersona)
- 말풍선/외형/대사/걷기 스프라이트 포함
- 멀티펫에서 프리셋으로 2번째 펫 불러오기

### 📊 대화 로그
- **직접 대화 로그**: 말걸기 → 펫 응답 (펫별 분리, 듀얼 모드 시 콤보키 저장)
- **채팅 반응 로그**: AI 응답에 대한 펫 반응 (채팅방별 분리)
- **펫끼리 대화 로그**: 자동 대화 기록 (이름 조합별 키)
- **자발적 말걸기 로그**: 트리거 라벨 `자발적 말걸기`로 기록
- 로그 뷰어 (필터: 전체/직접대화/채팅반응/펫끼리) / JSON 내보내기
- 무드 이모지 배지 표시
...
### 멀티펫
1. **설정** 탭 → 멀티펫 섹션 → **멀티펫 사용** ON
2. 미리 저장한 프리셋 중 하나를 2번째 펫으로 선택 → **불러오기**
3. 채팅 반응 분배: 1번째만 / 2번째만 / 번갈아가며
4. (선택) 듀얼 직접대화 ON → 말걸기 시 양쪽 동시 반응
5. (선택) 펫끼리 자동 대화 ON → 간격 설정
...
> ✅ 포함: 펫 이름 · 성격 · 관계 · 외형(이미지/크기) · 말풍선 · 대사  
> 🔒 제외: 유저 이름 · 유저 설정(페르소나) · API · 로그 등
...
### v1.2.0
- 🐱🐱 **멀티펫 시스템**: 2마리 동시 표시, 독립적 상호작용/컨디션/걷기/말풍선
- 🔄 **채팅 반응 분배**: 1번째만 / 2번째만 / 번갈아가며
- 💬 **듀얼 직접대화**: 말걸기 시 양쪽 펫 동시 반응 (1회 API 호출)
- 🗨️ **펫끼리 자동 대화**: 설정 간격마다 두 펫이 자동 대화 (로그 기록 + 기억)
- 💥 **펫 충돌 감지**: 겹치면 밀어내기 + 충돌 대사 (커스텀 가능)
- 🌐 **말투 언어 선택**: 한국어/영어/일본어/중국어
- ⏳ **API 충돌 처리**: 채팅 반응 최대 15초 대기 후 실행, 펫간 대화 30초 후 재시도
- 📊 **펫끼리 대화 로그**: 이름 조합별 키로 저장, 로그 뷰어 필터 추가
- 🎨 **UI 개편**: 탭 구조 정리 (프리셋 / 설정 / 로그), ON/OFF 토글 상단 분리, 공통 설정 섹션
- 🖼️ **2번째 펫 위치 프리셋**: 독립 위치 설정 가능
- 🔧 **리사이즈 시 2번째 펫 위치 보정**: 화면 밖 나가지 않도록 자동 clamp
- 🐛 `playHearts()` ReferenceError 크래시 수정 (쓰다듬기 100% 실패하던 문제)
- 🐛 듀얼모드 에러 시 상대 펫 thinking 상태 잔류 수정
- 🐛 듀얼모드 로그 콤보키(`isDual`) 누락 수정
- 🐛 채팅방 변경 시 `messageCounter` 미리셋 → 반응 뭉침 수정
- 🐛 `resetAllSettings` 시 2번째 펫 DOM/타이머 미정리 수정
- 🐛 2번째 펫 걷기 중 ST 생성 상태 체크 누락 수정
- 🐛 관전자 프롬프트 규칙 줄바꿈 누락 수정
- 📋 `state.js` 동적 속성 명시 선언 (코드 가독성 개선)
